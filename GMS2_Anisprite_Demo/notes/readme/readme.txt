...
you can define animation datasets containing data for several animations, assign that dataset to an 
Anisprite instance-struct, and dynamically switch between your predefined animations with a single method 
call. There are also a number of features that any 2D dev/animator is likely to find useful, such as manual
...
The final piece of the puzzle, Anisprite instance-structs themselves. As stated in the "USAGE" section, these should
be created using the Anisprite() constructor, updated using the step() method, and drawn using the draw() method. In
addition to all of these, I will also cover the additional methods/variables/features that will allow you to get the
most out of Anisprites here! See the __Anisprite_object script for source code
